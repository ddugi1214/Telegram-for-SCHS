...
public abstract class SampleSourceTrackRenderer extends TrackRenderer {
  ...
  protected final boolean doPrepare(long positionUs) throws ExoPlaybackException {
    ...
    for (int sourceIndex = 0; sourceIndex < sourceCount; sourceIndex++) {
      SampleSource.SampleSourceReader source = sources[sourceIndex];
  ...
  protected final MediaFormat getFormat(int track) {
    SampleSource.SampleSourceReader source = sources[handledSourceIndices[track]];
   ...
   *     {@link SampleSource.SampleSourceReader#continueBuffering(int, long)}.
