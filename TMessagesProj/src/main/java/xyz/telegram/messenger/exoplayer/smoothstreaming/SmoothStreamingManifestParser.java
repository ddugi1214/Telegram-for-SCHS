...
import xyz.telegram.messenger.exoplayer.extractor.mp4.PsshAtomUtil;
import xyz.telegram.messenger.exoplayer.upstream.UriLoadable;
...
public class SmoothStreamingManifestParser implements UriLoadable.Parser<SmoothStreamingManifest> {
  ...
  private static class SmoothStreamMediaParser extends ElementParser {
    ...
    public void addChild(Object child) {
      if (child instanceof SmoothStreamingManifest.StreamElement) {
        streamElements.add((SmoothStreamingManifest.StreamElement) child);
      } else if (child instanceof SmoothStreamingManifest.ProtectionElement) {
        Assertions.checkState(protectionElement == null);
        protectionElement = (SmoothStreamingManifest.ProtectionElement) child;
      }
    }

    @Override
    public Object build() {
      SmoothStreamingManifest.StreamElement[] streamElementArray = new SmoothStreamingManifest.StreamElement[streamElements.size()];
  ...
  private static class StreamElementParser extends ElementParser {
    ...
    private void parseStreamElementStartTag(XmlPullParser parser) throws ParserException {
      ...
      if (type == SmoothStreamingManifest.StreamElement.TYPE_TEXT) {
    ...
    private int parseType(XmlPullParser parser) throws ParserException {
      ...
      if (value != null) {
        if (KEY_TYPE_AUDIO.equalsIgnoreCase(value)) {
          return SmoothStreamingManifest.StreamElement.TYPE_AUDIO;
        } else if (KEY_TYPE_VIDEO.equalsIgnoreCase(value)) {
          return SmoothStreamingManifest.StreamElement.TYPE_VIDEO;
        } else if (KEY_TYPE_TEXT.equalsIgnoreCase(value)) {
          return SmoothStreamingManifest.StreamElement.TYPE_TEXT;
        } else {
    ...
    public void addChild(Object child) {
      if (child instanceof SmoothStreamingManifest.TrackElement) {
        tracks.add((SmoothStreamingManifest.TrackElement) child);
      }
    }

    @Override
    public Object build() {
      SmoothStreamingManifest.TrackElement[] trackElements = new SmoothStreamingManifest.TrackElement[tracks.size()];
      tracks.toArray(trackElements);
      return new SmoothStreamingManifest.StreamElement(baseUri, url, type, subType, timescale, name, qualityLevels,
  ...
  private static class TrackElementParser extends ElementParser {
    ...
    public void parseStartTag(XmlPullParser parser) throws ParserException {
      ...
      if (type == SmoothStreamingManifest.StreamElement.TYPE_VIDEO) {
        ...
        mimeType = fourCC != null ? fourCCToMimeType(fourCC)
            : type == SmoothStreamingManifest.StreamElement.TYPE_AUDIO ? MimeTypes.AUDIO_AAC : null;
      }

      if (type == SmoothStreamingManifest.StreamElement.TYPE_AUDIO) {
    ...
    public Object build() {
      ...
      return new SmoothStreamingManifest.TrackElement(index, bitrate, mimeType, csdArray, maxWidth, maxHeight, samplingRate,
          channels, language);
    }
