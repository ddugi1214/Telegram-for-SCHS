...
public class IdenticonActivity extends BaseFragment {
    ...
    public View createView(Context context) {
        ...
        actionBar.setTitle(LocaleController.getString("EncryptionKey", xyz.telegram.messenger.R.string.EncryptionKey));
        ...
        if (encryptedChat != null) {
            ...
            hash.append(AndroidUtilities.replaceTags(LocaleController.formatString("EncryptionKeyDescription", xyz.telegram.messenger.R.string.EncryptionKeyDescription, user.first_name, user.first_name)));
            final String url = "telegram.org";
            int index = hash.toString().indexOf(url);
            if (index != -1) {
                hash.setSpan(new URLSpanReplacement(LocaleController.getString("EncryptionKeyLink", xyz.telegram.messenger.R.string.EncryptionKeyLink)), index, index + url.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
