...
import xyz.telegram.ui.Components.LinkPath;
import xyz.telegram.ui.Components.RadialProgress;
import xyz.telegram.ui.Components.SeekBar;
import xyz.telegram.ui.Components.SeekBarWaveform;
import xyz.telegram.ui.Components.StaticLayoutEx;
import xyz.telegram.ui.ActionBar.Theme;
import xyz.telegram.ui.Components.TypefaceSpan;
...
public class ChatMessageCell extends BaseCell implements SeekBar.SeekBarDelegate, ImageReceiver.ImageReceiverDelegate, MediaController.FileDownloadProgressListener {
        ...
        if (messageChanged || dataChanged || isPhotoDataChanged(messageObject)) {
            ...
            if (messageObject.type == 0) {
                ...
                if (phone != null && phone.length() != 0) {
                    ...
                    phone = LocaleController.getString("NumberUnknown", xyz.telegram.messenger.R.string.NumberUnknown);
                }
    ...
    private void measureTime(MessageObject messageObject) {
        ...
        if (messageObject.messageOwner.via_bot_id == 0 && messageObject.messageOwner.via_bot_name == null && (author == null || !author.bot) && (messageObject.messageOwner.flags & TLRPC.MESSAGE_FLAG_EDITED) != 0) {
            timeString = LocaleController.getString("EditedMessage", xyz.telegram.messenger.R.string.EditedMessage) + " " + LocaleController.getInstance().formatterDay.format((long) (messageObject.messageOwner.date) * 1000);
        } else {
    ...
    private void setMessageObjectInternal(MessageObject messageObject) {
        ...
        if (drawForwardedName && messageObject.isForwarded()) {
            ...
            if (currentForwardUser != null || currentForwardChannel != null) {
                ...
                int fromWidth = (int) Math.ceil(forwardNamePaint.measureText(LocaleController.getString("From", xyz.telegram.messenger.R.string.From) + " "));
                ...
                if (viaString != null) {
                    viaNameWidth = (int) Math.ceil(forwardNamePaint.measureText(LocaleController.getString("From", xyz.telegram.messenger.R.string.From) + " " + name));
                    lastLine = AndroidUtilities.replaceTags(String.format("%s <b>%s</b> via <b>%s</b>", LocaleController.getString("From", xyz.telegram.messenger.R.string.From), name, viaUsername));
                } else {
                    lastLine = AndroidUtilities.replaceTags(String.format("%s <b>%s</b>", LocaleController.getString("From", xyz.telegram.messenger.R.string.From), name));
                }
                ...
                try {
                    ...
                    lastLine = TextUtils.ellipsize(AndroidUtilities.replaceTags(LocaleController.getString("ForwardedMessage", xyz.telegram.messenger.R.string.ForwardedMessage)), forwardNamePaint, forwardedNameWidth, TextUtils.TruncateAt.END);
        ...
        if (messageObject.isReply()) {
            ...
            if (stringFinalName == null) {
                stringFinalName = LocaleController.getString("Loading", xyz.telegram.messenger.R.string.Loading);
            }
            try {
