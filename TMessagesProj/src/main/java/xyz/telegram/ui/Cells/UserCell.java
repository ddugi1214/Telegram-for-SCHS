...
import xyz.telegram.ui.Components.CheckBoxSquare;
...
public class UserCell extends FrameLayout {
    ...
    public void setIsAdmin(int value) {
        ...
        if (value == 1) {
            adminImage.setImageResource(xyz.telegram.messenger.R.drawable.admin_star);
        } else if (value == 2) {
            adminImage.setImageResource(xyz.telegram.messenger.R.drawable.admin_star2);
        }
    }
        ...
        if (currrntStatus != null) {
            ...
            if (currentUser.bot) {
                ...
                if (currentUser.bot_chat_history || adminImage != null && adminImage.getVisibility() == VISIBLE) { //TODO fix
                    statusTextView.setText(LocaleController.getString("BotStatusRead", xyz.telegram.messenger.R.string.BotStatusRead));
                } else {
                    statusTextView.setText(LocaleController.getString("BotStatusCantRead", xyz.telegram.messenger.R.string.BotStatusCantRead));
                }
            } else {
                if (currentUser.id == UserConfig.getClientUserId() || currentUser.status != null && currentUser.status.expires > ConnectionsManager.getInstance().getCurrentTime() || MessagesController.getInstance().onlinePrivacy.containsKey(currentUser.id)) {
                    ...
                    statusTextView.setText(LocaleController.getString("Online", xyz.telegram.messenger.R.string.Online));
                } else {
                    statusTextView.setTextColor(statusColor);
