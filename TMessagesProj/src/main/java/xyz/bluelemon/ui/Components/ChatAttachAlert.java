...
import xyz.bluelemon.ui.Cells.PhotoAttachCameraCell;
import xyz.bluelemon.ui.Cells.PhotoAttachPhotoCell;
import xyz.bluelemon.ui.Cells.ShadowSectionCell;
import xyz.bluelemon.ui.ChatActivity;
import xyz.bluelemon.ui.PhotoViewer;
...
public class ChatAttachAlert extends BottomSheet implements NotificationCenter.NotificationCenterDelegate, PhotoViewer.PhotoViewerProvider, BottomSheet.BottomSheetDelegateInterface {
    ...
    private class AttachBotButton extends FrameLayout {
        ...
        private void onLongPress() {
            ...
            builder.setMessage(LocaleController.formatString("ChatHintsDelete", xyz.bluelemon.tfctalk.R.string.ChatHintsDelete, ContactsController.formatName(currentUser.first_name, currentUser.last_name)));
    ...
    public ChatAttachAlert(Context context) {
        ...
        shadowDrawable = context.getResources().getDrawable(xyz.bluelemon.tfctalk.R.drawable.sheet_shadow);
        ...
        if (Build.VERSION.SDK_INT >= 23 && getContext().checkSelfPermission(Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            progressView.setText(LocaleController.getString("PermissionStorage", xyz.bluelemon.tfctalk.R.string.PermissionStorage));
            progressView.setTextSize(16);
        } else {
            progressView.setText(LocaleController.getString("NoPhotos", xyz.bluelemon.tfctalk.R.string.NoPhotos));
        ...
        CharSequence[] items = new CharSequence[]{
                LocaleController.getString("ChatCamera", xyz.bluelemon.tfctalk.R.string.ChatCamera),
                LocaleController.getString("ChatGallery", xyz.bluelemon.tfctalk.R.string.ChatGallery),
                LocaleController.getString("ChatVideo", xyz.bluelemon.tfctalk.R.string.ChatVideo),
                LocaleController.getString("AttachMusic", xyz.bluelemon.tfctalk.R.string.AttachMusic),
                LocaleController.getString("ChatDocument", xyz.bluelemon.tfctalk.R.string.ChatDocument),
                LocaleController.getString("AttachContact", xyz.bluelemon.tfctalk.R.string.AttachContact),
                LocaleController.getString("ChatLocation", xyz.bluelemon.tfctalk.R.string.ChatLocation),
                ""
        };
        for (int a = 0; a < 8; a++) {
        ...
        hintTextView.setBackgroundResource(xyz.bluelemon.tfctalk.R.drawable.tooltip);
        ...
        hintTextView.setText(LocaleController.getString("AttachBotsHelp", xyz.bluelemon.tfctalk.R.string.AttachBotsHelp));
        hintTextView.setGravity(Gravity.CENTER_VERTICAL);
        hintTextView.setVisibility(View.INVISIBLE);
        hintTextView.setCompoundDrawablesWithIntrinsicBounds(xyz.bluelemon.tfctalk.R.drawable.scroll_tip, 0, 0, 0);
    ...
    public void updatePhotosButton() {
        ...
        if (count == 0) {
            sendPhotosButton.imageView.setPadding(0, AndroidUtilities.dp(4), 0, 0);
            sendPhotosButton.imageView.setBackgroundResource(xyz.bluelemon.tfctalk.R.drawable.attach_hide_states);
            sendPhotosButton.imageView.setImageResource(xyz.bluelemon.tfctalk.R.drawable.attach_hide2);
            sendPhotosButton.textView.setText("");
        } else {
            sendPhotosButton.imageView.setPadding(AndroidUtilities.dp(2), 0, 0, 0);
            sendPhotosButton.imageView.setBackgroundResource(xyz.bluelemon.tfctalk.R.drawable.attach_send_states);
            sendPhotosButton.imageView.setImageResource(xyz.bluelemon.tfctalk.R.drawable.attach_send2);
            sendPhotosButton.textView.setText(LocaleController.formatString("SendItems", xyz.bluelemon.tfctalk.R.string.SendItems, String.format("(%d)", count)));
        }
    ...
    public void updatePhotoAtIndex(int index) {
        ...
        if (cell != null) {
            ...
            if (photoEntry.thumbPath != null) {
                cell.getImageView().setImage(photoEntry.thumbPath, null, cell.getContext().getResources().getDrawable(xyz.bluelemon.tfctalk.R.drawable.nophotos));
            } else if (photoEntry.path != null) {
                cell.getImageView().setOrientation(photoEntry.orientation, true);
                cell.getImageView().setImage("thumb://" + photoEntry.imageId + ":" + photoEntry.path, null, cell.getContext().getResources().getDrawable(xyz.bluelemon.tfctalk.R.drawable.nophotos));
            } else {
                cell.getImageView().setImageResource(xyz.bluelemon.tfctalk.R.drawable.nophotos);
            }
        }
    }
    ...
    private void startRevealAnimation(final boolean open) {
        ...
        if (open) {
            ...
            for (int a = 0; a < count; a++) {
                ...
                views[a].setTag(xyz.bluelemon.tfctalk.R.string.AppName, 1);
