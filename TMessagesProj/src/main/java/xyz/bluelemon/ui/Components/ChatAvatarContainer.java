...
import xyz.bluelemon.ui.ProfileActivity;
            ...
            if (chat != null) {
                ...
                if (ChatObject.isChannel(chat)) {
                    if (info != null && info.participants_count != 0) {
                        ...
                        if (chat.megagroup) {
                            subtitleTextView.setText(LocaleController.getString("Loading", xyz.bluelemon.tfctalk.R.string.Loading).toLowerCase());
                        } else {
                            if ((chat.flags & TLRPC.CHAT_FLAG_IS_PUBLIC) != 0) {
                                subtitleTextView.setText(LocaleController.getString("ChannelPublic", xyz.bluelemon.tfctalk.R.string.ChannelPublic).toLowerCase());
                            } else {
                                subtitleTextView.setText(LocaleController.getString("ChannelPrivate", xyz.bluelemon.tfctalk.R.string.ChannelPrivate).toLowerCase());
                            }
                        }
                    }
                } else {
                    if (ChatObject.isKickedFromChat(chat)) {
                        subtitleTextView.setText(LocaleController.getString("YouWereKicked", xyz.bluelemon.tfctalk.R.string.YouWereKicked));
                    } else if (ChatObject.isLeftFromChat(chat)) {
                        subtitleTextView.setText(LocaleController.getString("YouLeft", xyz.bluelemon.tfctalk.R.string.YouLeft));
                    } else {
                ...
                if (user.id == 333000 || user.id == 777000) {
                    newStatus = LocaleController.getString("ServiceNotifications", xyz.bluelemon.tfctalk.R.string.ServiceNotifications);
                } else if (user.bot) {
                    newStatus = LocaleController.getString("Bot", xyz.bluelemon.tfctalk.R.string.Bot);
                } else {
