...
import xyz.bluelemon.ui.Cells.LocationCell;
import xyz.bluelemon.ui.Cells.LocationLoadingCell;
import xyz.bluelemon.ui.Cells.LocationPoweredCell;
import xyz.bluelemon.ui.Cells.SendLocationCell;
...
public class LocationActivityAdapter extends BaseLocationAdapter {
    ...
    private void updateCell() {
        if (sendLocationCell != null) {
            if (customLocation != null) {
                sendLocationCell.setText(LocaleController.getString("SendSelectedLocation", xyz.bluelemon.tfctalk.R.string.SendSelectedLocation), String.format(Locale.US, "(%f,%f)", customLocation.getLatitude(), customLocation.getLongitude()));
            } else {
                if (gpsLocation != null) {
                    sendLocationCell.setText(LocaleController.getString("SendLocation", xyz.bluelemon.tfctalk.R.string.SendLocation), LocaleController.formatString("AccurateTo", xyz.bluelemon.tfctalk.R.string.AccurateTo, LocaleController.formatPluralString("Meters", (int) gpsLocation.getAccuracy())));
                } else {
                    sendLocationCell.setText(LocaleController.getString("SendLocation", xyz.bluelemon.tfctalk.R.string.SendLocation), LocaleController.getString("Loading", xyz.bluelemon.tfctalk.R.string.Loading));
                }
            }
        }
    }
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        if (i == 0) {
            ...
            ((GreySectionCell) view).setText(LocaleController.getString("NearbyPlaces", xyz.bluelemon.tfctalk.R.string.NearbyPlaces));
