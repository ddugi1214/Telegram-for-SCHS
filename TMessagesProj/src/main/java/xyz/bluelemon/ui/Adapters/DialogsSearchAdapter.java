...
import xyz.bluelemon.ui.Cells.HashtagSearchCell;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            case 0:
                ...
                view.setBackgroundResource(xyz.bluelemon.tfctalk.R.drawable.list_selector);
                break;
            case 1:
        ...
        switch (holder.getItemViewType()) {
            ...
            case 1: {
                ...
                if (isRecentSearchDisplayed()) {
                    int offset = (!SearchQuery.hints.isEmpty() ? 2 : 0);
                    if (position < offset) {
                        cell.setText(LocaleController.getString("ChatHints", xyz.bluelemon.tfctalk.R.string.ChatHints).toUpperCase());
                    } else {
                        cell.setText(LocaleController.getString("Recent", xyz.bluelemon.tfctalk.R.string.Recent).toUpperCase());
                    }
                } else if (!searchResultHashtags.isEmpty()) {
                    cell.setText(LocaleController.getString("Hashtags", xyz.bluelemon.tfctalk.R.string.Hashtags).toUpperCase());
                } else if (!globalSearch.isEmpty() && position == searchResult.size()) {
                    cell.setText(LocaleController.getString("GlobalSearch", xyz.bluelemon.tfctalk.R.string.GlobalSearch));
                } else {
                    cell.setText(LocaleController.getString("SearchMessages", xyz.bluelemon.tfctalk.R.string.SearchMessages));
                }
                break;
            }
            case 2: {
