...
package xyz.bluelemon.ui.Cells;
...
import xyz.bluelemon.ui.Components.LinkPath;
import xyz.bluelemon.ui.Components.RadialProgress;
import xyz.bluelemon.ui.Components.SeekBar;
import xyz.bluelemon.ui.Components.SeekBarWaveform;
import xyz.bluelemon.ui.Components.StaticLayoutEx;
import xyz.bluelemon.ui.ActionBar.Theme;
import xyz.bluelemon.ui.Components.TypefaceSpan;
...
public class ChatMessageCell extends BaseCell implements SeekBar.SeekBarDelegate, ImageReceiver.ImageReceiverDelegate, MediaController.FileDownloadProgressListener {
        ...
        if (messageChanged || dataChanged || isPhotoDataChanged(messageObject)) {
            ...
            if (messageObject.type == 0) {
                ...
                if (phone != null && phone.length() != 0) {
                    ...
                    phone = LocaleController.getString("NumberUnknown", xyz.bluelemon.tfctalk.R.string.NumberUnknown);
                }
            ...
            if (captionLayout == null && messageObject.caption != null && messageObject.type != 13) {
                try {
                    ...
                    captionLayout = new StaticLayout(messageObject.caption, MessageObject.getTextPaint(), width - AndroidUtilities.dp(10), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                    if (captionLayout.getLineCount() > 0) {
            ...
            if (messageObject.messageOwner.reply_markup instanceof TLRPC.TL_replyInlineMarkup) {
                ...
                for (int a = 0; a < rows; a++) {
                    ...
                    for (int b = 0; b < row.buttons.size(); b++) {
                        ...
                        CharSequence buttonText = Emoji.replaceEmoji(botButton.button.text, botButtonPaint.getFontMetricsInt(), AndroidUtilities.dp(15), false);
    ...
    private void drawContent(Canvas canvas) {
        ...
        if (captionLayout != null) {
            ...
            if (currentMessageObject.type == 1 || documentAttachType == DOCUMENT_ATTACH_TYPE_VIDEO || currentMessageObject.type == 8) {
                ...
                canvas.translate(captionX = currentBackgroundDrawable.getBounds().left + AndroidUtilities.dp(currentMessageObject.isOutOwner() ? 11 : 17), captionY = totalHeight - captionHeight - AndroidUtilities.dp(10));
            }
            if (pressedLink != null) {
                for (int b = 0; b < urlPath.size(); b++) {
                    canvas.drawPath(urlPath.get(b), urlPaint);
    ...
    private void measureTime(MessageObject messageObject) {
        ...
        if (messageObject.messageOwner.via_bot_id == 0 && messageObject.messageOwner.via_bot_name == null && (author == null || !author.bot) && (messageObject.messageOwner.flags & TLRPC.MESSAGE_FLAG_EDITED) != 0) {
            timeString = LocaleController.getString("EditedMessage", xyz.bluelemon.tfctalk.R.string.EditedMessage) + " " + LocaleController.getInstance().formatterDay.format((long) (messageObject.messageOwner.date) * 1000);
        } else {
    ...
    private void setMessageObjectInternal(MessageObject messageObject) {
        ...
        if (drawForwardedName && messageObject.isForwarded()) {
            ...
            if (currentForwardUser != null || currentForwardChannel != null) {
                ...
                int fromWidth = (int) Math.ceil(forwardNamePaint.measureText(LocaleController.getString("From", xyz.bluelemon.tfctalk.R.string.From) + " "));
                ...
                if (viaString != null) {
                    viaNameWidth = (int) Math.ceil(forwardNamePaint.measureText(LocaleController.getString("From", xyz.bluelemon.tfctalk.R.string.From) + " " + name));
                    lastLine = AndroidUtilities.replaceTags(String.format("%s <b>%s</b> via <b>%s</b>", LocaleController.getString("From", xyz.bluelemon.tfctalk.R.string.From), name, viaUsername));
                } else {
                    lastLine = AndroidUtilities.replaceTags(String.format("%s <b>%s</b>", LocaleController.getString("From", xyz.bluelemon.tfctalk.R.string.From), name));
                }
                ...
                try {
                    ...
                    lastLine = TextUtils.ellipsize(AndroidUtilities.replaceTags(LocaleController.getString("ForwardedMessage", xyz.bluelemon.tfctalk.R.string.ForwardedMessage)), forwardNamePaint, forwardedNameWidth, TextUtils.TruncateAt.END);
        ...
        if (messageObject.isReply()) {
            ...
            if (stringFinalName == null) {
                stringFinalName = LocaleController.getString("Loading", xyz.bluelemon.tfctalk.R.string.Loading);
            }
            try {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (drawShareButton) {
            ...
            if (currentMessageObject.isOutOwner()) {
                shareStartX = currentBackgroundDrawable.getBounds().left - AndroidUtilities.dp(8) - Theme.shareDrawable.getIntrinsicWidth();
            } else {
                shareStartX = currentBackgroundDrawable.getBounds().right + AndroidUtilities.dp(8);
            }
            setDrawableBounds(Theme.shareDrawable, shareStartX, shareStartY = layoutHeight - AndroidUtilities.dp(41));
